 ...
 * 串口管理类
 */
public class Serial485Manager extends SerialHelper {
    private static final String TAG = "Serial485Manager";
    private static volatile Serial485Manager instance;
    ...
    private Serial485Manager() {
        mSerialManagerHandler = new SerialManagerHandler();
        dataBuilder = new StringBuilder();
        config(sPort,iBaudRate);
    }

    public static Serial485Manager getInstance() {
        if (instance == null) {
            synchronized (Serial485Manager.class) {
                if (instance == null) {
                    instance = new Serial485Manager();
                }
            }
        }
        return instance;
